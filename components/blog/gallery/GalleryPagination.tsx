...
import { Pagination } from "../../../std/commerce/butterCMS/types.ts";
...
export default function GalleryPagination({ pagination, containerId }: Props) {
  ...
  const btnClasses =
    tw`relative w-10 h-10 flex justify-center items-center cursor-pointer disabled:(opacity-[10%] cursor-default)`;
  const blackBtnClasses = tw`bg-black rounded text-white`;

  const iconsClasses = tw`absolute left-[0.15rem]`;
  const iconsParams = {
    size: 24,
    strokeWidth: 2,
  };
